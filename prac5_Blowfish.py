...
P_block=np.array(["243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822",
                   "299f31d0", "082efa98", "ec4e6c89", "452821e6", "38d01377",
                   "be5466cf", "34e90c6c", "c0ac29b7", "c97c50dd", "3f84d5b5",
                   "b5470917", "9216d5d9", "8979fb1b"])

S_block=np.array([["d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed",
...
class Solution:
    ...
    def InitializeText(self,text):
        for i in range(64):
            text+="0"
        return text    
    
    def Convert2Binary(self,hexaD):
        _str = "".join(['{0:04b}'.format(int(d,16)) for d in hexaD])
        return _str
    
    def a_XOR_b(self,a,b):
        return ''.join('1' if b1 != b2 else '0' for b1, b2 in zip(a, b))
    ...
    def Encrypt(self,text):
        ...
        R=text[32:]
        for i in range(16):
            p=self.Convert2Binary(P_block[i])
            X=self.a_XOR_b(L,p)
            Y=X[:]
            s1=int(self.Convert2Binary(S_block[0,int(X[:8],2)]),2)
            s2=int(self.Convert2Binary(S_block[1,int(X[8:16],2)]),2)
            s3=int(self.Convert2Binary(S_block[2,int(X[16:24],2)]),2)
            s4=int(self.Convert2Binary(S_block[3,int(X[24:32],2)]),2)
            f=((((s1+s2)%(2**32))^s3)+s4)%(2**32)
            f='{:032b}'.format(f)
            Z=self.a_XOR_b(f,R)
            R=Y[:]
            L=Z[:]
          
        p=self.Convert2Binary(P_block[16])
        X=self.a_XOR_b(L,p)    
        p=self.Convert2Binary(P_block[17])
        Y=self.a_XOR_b(R,p)      
        return Y+X
    ...
    def Decrypt(self,text):
        ...
        p=self.Convert2Binary(P_block[1])
        X=self.a_XOR_b(L,p)    
        p=self.Convert2Binary(P_block[0])
        Y=self.a_XOR_b(R,p)      
        return Y+X
    ...
    def E_PT(self,PT,key):
        l=len(key)//32
        j=0
        for idx, i in enumerate(P_block):
            if j>=l:
                j=0
            x=self.Convert2Binary(i)  
            y=key[j*32:(j*32)+32] 
            P_block[idx]=str(hex(int(self.a_XOR_b(x,y),2)))[2:].rjust(8,"0")
            j+=1  

        text=self.InitializeText("")    
        for i in range(9):
            a=self.Encrypt(text)
            P_block[i*2]=str(hex(int(a[:32],2)))[2:].rjust(8, "0")
            P_block[(i*2)+1]=str(hex(int(a[32:],2)))[2:].rjust(8, "0")
            text=a[:]    
        ...
        for i in range(4):
            for j in range(128):
                a=self.Encrypt(text)
                S_block[i,j*2]=str(hex(int(a[:32],2)))[2:].rjust(8, "0")
                S_block[i,(j*2)+1]=str(hex(int(a[32:],2)))[2:].rjust(8, "0")
                text=a[:]
        CT=""
        m=len(PT)//64
        for i in range(m):
            CT+=self.Encrypt(PT[i*64:(i*64)+64])   
        CT=hex(int(CT,2))[2:]   
        return CT  
    ...
    def D_CT(self,CT):   
        ...
        m=len(CT)//64
        for i in range(m):
            PT+=self.Decrypt(CT[i*64:(i*64)+64])   
        PT=hex(int(PT,2))[2:]
        return PT
...
# key max size should be 56 characters
